...
export const CardView: React.FC<CardViewProps> = ({
  ...
  const sizeClasses = {
    ...
    sm: 'w-[42px] h-[60px] xs:w-11 xs:h-16 sm:w-13 sm:h-18 text-xs rounded-lg p-1',
  ...
  if (isFaceDown || !card) {
    return (
      <div
        className={`${sizeClasses} bg-blue-900 border-2 border-blue-700 shadow-md relative overflow-hidden flex items-center justify-center select-none touch-manipulation ${className}`}
      >
  ...
  if (card.isJoker) {
    return (
      <div
        ...
        className={`
          ${sizeClasses}
          relative select-none touch-manipulation transition-all duration-200
          ...
          ${
            isSelected
              ...
              : isLegal && onClick
              ? 'hover:-translate-y-1 active:scale-95 hover:shadow-md cursor-pointer z-10'
              : 'z-10'
          }
          ${className}
        `}
